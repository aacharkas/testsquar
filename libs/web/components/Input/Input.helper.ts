...
export const inputHelper = (error: boolean): string => {
  return !error
    ? 'border-gray-300 shadow-sm focus:border-indigo-700 focus:ring-indigo-700'
    : 'border-red-300 pr-10 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500';
};
...
export const useInputMask = (mask = '') => {
  ...
  useEffect(() => {
    ...
    inputEl.setAttribute('placeholder', 'MM/DD');
  }, [inputEl]);
